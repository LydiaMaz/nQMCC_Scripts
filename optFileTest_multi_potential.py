...
def BoundStateOptimize(target: wavefunction_t, util: utility_t, pot_pair_name: str, pot_dir: str):
    ...
    log_initial = f"{pot_dir}logs/{pot_pair_name}.initial"
    ...
    # Step 2: Set up optimization instructions
    print("Step 2: SETTING UP OPTIMIZATION INSTRUCTIONS")
    
    # Instructions for all correlations
    instructions_corr = [
    ...
    # Combined instructions for full optimization
    instructions_all = instructions_corr + instructions_ss
    
    print("... DONE")
    print(BREAK)
    
    # Step 3: Optimize all correlations together
    print("Step 3: CORRELATION OPTIMIZATION")
    opt_all_file_name = f"'{pot_dir}opt/{pot_pair_name}_all.opt'"
    opt_all = GenerateOptFile(target.PARAMS, target.DK, opt_all_file_name, instructions_all)
    opt_all.UpdateFloats(target.PARAMS, 6)
    
    dk_name_all = f"'{pot_dir}dk/{pot_pair_name}_optimized.dk'"
    log_optimize_all = f"{pot_dir}logs/{pot_pair_name}.optimize"
    print(f"BEGIN CORRELATION OPTIMIZATION: {log_optimize_all}.optimize")
    
    e_optimized, v_optimized = target.Optimize(opt_all, dk_name_all, True, log_optimize_all)
    
    print(f" ⚛ OPTIMIZED E = {e_optimized:.4f} +- {v_optimized:.4f}")
    print(f"TOTAL IMPROVEMENT: {e_optimized - e_initial:.4f} MeV")
